<template>
  <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
    ...
    <router-link v-if="item.to && !item.items && item.visible !== false" @click="itemClick($event, item)"
      :class="[item.class, { 'active-route': appStore.checkActiveRoute(item) }]" tabindex="0" :to="item.to">
      <i :class="item.icon" class="layout-menuitem-icon"></i>
      <span class="layout-menuitem-text">{{ item.label }}</span>
      <i class="pi pi-fw pi-angle-down layout-submenu-toggler" v-if="item.items"></i>
    </router-link>
...
<script setup lang="ts">
  ...
  const itemClick = (event: any, item: any) => {
    ...
    if ((item.to || item.url) && (appStore.staticMenuMobileActive || appStore.overlayMenuActive)) {
